...
# LEVEL 2 SPECIFIC
	if ($LevelTiling.position.x == -1024 && !secondScreenReached):
		secondScreenReached = true
		print ("woaoaooa")
		var cbArray = [0,0,0,0,0]
		for n in range (0,4):
			cbArray[n] = $'..'.cloudBox.instance()
			var new_x = 384 + n*70
			var new_y = 320
			cbArray[n].set_position_x(new_x)
			cbArray[n].set_position_y(new_y)
			add_child(cbArray[n])
